...
export default () => {
  return (dispatch) => {
    recipes.on('created', (recipe) => {
      dispatch(createdRecipe(recipe))
    })
    recipes.on('updated', (recipe) => {
      dispatch(updatedRecipe(recipe))
    })
    recipes.on('patched', (recipe) => {
      dispatch(updatedRecipe(recipe))
    })
    recipes.on('removed', (recipe) => {
      dispatch(removedRecipe(recipe))
    })
